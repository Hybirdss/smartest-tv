...
이런 걸 직접 다룰 필요 없습니다. 드라이버가 알아서 합니다.
...
18개 도구 제공: `tv_on`, `tv_off`, `tv_launch`, `tv_close`, `tv_volume`, `tv_set_volume`, `tv_mute`, `tv_play`, `tv_pause`, `tv_stop`, `tv_status`, `tv_info`, `tv_notify`, `tv_apps`, `tv_volume_up`, `tv_volume_down`, `tv_screen_on`, `tv_screen_off`.

설정은 `~/.config/smartest-tv/config.toml`에서 자동으로 읽습니다 — 환경변수 불필요.
