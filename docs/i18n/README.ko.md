...
stv launch netflix 82656797         # 특정 콘텐츠로 딥링크
stv launch youtube dQw4w9WgXcQ     # 유튜브 영상 재생
stv launch spotify spotify:album:x  # Spotify 재생
stv volume 25                       # 볼륨 설정
stv mute                            # 음소거 토글
...
| `tv-netflix` | Playwright 스크래핑으로 에피소드 ID 조회 |
...
```
사용자 (자연어)
  → AI + 스킬 (yt-dlp / Playwright / 웹 검색으로 콘텐츠 ID 탐색)
    → stv CLI (포맷 변환 및 전송)
