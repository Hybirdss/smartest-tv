...
**오후 9시** -- 친구들이 온다. 다들 `stv queue add ...` 실행 -- TV가 순서대로 재생.
...
| **모든 MCP 클라이언트** | stdio 또는 HTTP로 32개 도구 사용 |
...
| [MCP 도구](../../docs/reference/mcp-tools.md) | 파라미터 포함 MCP 도구 32개 전체 |
...
```bash
pip install -e ".[dev]"
python -m pytest tests/ -v         # 132개 테스트, TV 불필요
```
