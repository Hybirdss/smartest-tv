...
def _mock_curl(html: str):
    """Return a mock subprocess.run result with the given html."""
    mock = MagicMock()
    mock.stdout = html
    mock.returncode = 0
    return mock
...
@patch("smartest_tv.resolve.subprocess.run")
def test_scrape_episodes_only_no_season_ids(mock_run):
    ...
    mock_run.return_value = _mock_curl(_make_html(ep_ids, season_ids))
...
@patch("smartest_tv.resolve.subprocess.run")
def test_scrape_no_episodes_in_html_returns_empty(mock_run):
...
@patch("smartest_tv.resolve.subprocess.run")
def test_scrape_timeout_returns_empty(mock_run):
    import subprocess
...
@patch("smartest_tv.resolve.subprocess.run")
def test_scrape_oserror_returns_empty(mock_run):
