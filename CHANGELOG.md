...
- **A stuck TV command blocked the REST API forever.** v1.3.0 made the
  serve API threaded, but the driver-serialization lock was acquired
  without a timeout: one hung command (pathological TLS handshake, dead
  ...
  default 30 s, invalid values fall back to the default) and latecomers
  get an explicit **HTTP 503** `DriverBusyError` instead of hanging.
